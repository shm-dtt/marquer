// Load bookmark folders on popup open
...
async function loadBookmarkFolders() {
  try {
    ...
    folderSelect.innerHTML = '<option value="">Select a folder to export</option>';
      ...
      nodes.forEach(node => {
        if (node.children) {
          ...
          if (node.children.length > 0) {
            addFoldersToSelect(node.children, prefix + (node.parentId ? '  ' : ''));
          }
        }
      });
    }
...
async function exportBookmarks() {
  ...
  try {
    ...
    const filename = `bookmarks_${folderData.title.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.json`;
    ...
    exportBtn.innerHTML = 'Export Selected Folder';
...
async function importBookmarks() {
  ...
  try {
    ...
    showStatus('importStatus', `Successfully imported ${importData.metadata?.totalBookmarks || 'unknown'} bookmarks!`, 'success');
    ...
    showStatus('importStatus', 'Error importing bookmarks: ' + error.message, 'error');
  } finally {
    importBtn.innerHTML = 'Import Bookmark Collection';
